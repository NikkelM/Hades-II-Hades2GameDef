---@meta game-Scripts-KeepsakeData
local game = {}

---@class game-Scripts-KeepsakeData*GiftData : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-KeepsakeData*GiftData
game.GiftData = ...
---@alias game-Scripts-KeepsakeData.GiftData ...

---@class game-Scripts-KeepsakeData*GiftIconData : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-KeepsakeData*GiftIconData
game.GiftIconData = ...
---@alias game-Scripts-KeepsakeData.GiftIconData ...

return game
