---@meta game-Scripts-HeroData
local game = {}

---@class game-Scripts-HeroData*GameOutroData : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-HeroData*GameOutroData
game.GameOutroData = ...
---@alias game-Scripts-HeroData.GameOutroData ...

---@class game-Scripts-HeroData*GameOutroPriorities : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-HeroData*GameOutroPriorities
game.GameOutroPriorities = ...
---@alias game-Scripts-HeroData.GameOutroPriorities ...

---@class game-Scripts-HeroData*GlobalModifiers : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-HeroData*GlobalModifiers
game.GlobalModifiers = ...
---@alias game-Scripts-HeroData.GlobalModifiers ...

---@class game-Scripts-HeroData*HeroData : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-HeroData*HeroData
game.HeroData = ...
---@alias game-Scripts-HeroData.HeroData ...

---@class game-Scripts-HeroData*HeroVoiceLines : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-HeroData*HeroVoiceLines
game.HeroVoiceLines = ...
---@alias game-Scripts-HeroData.HeroVoiceLines ...

return game
