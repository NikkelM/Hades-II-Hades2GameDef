---@meta game-Scripts-RewardData
local game = {}

---@class game-Scripts-RewardData*RewardData : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-RewardData*RewardData
game.RewardData = ...
---@alias game-Scripts-RewardData.RewardData ...

---@class game-Scripts-RewardData*RewardSets : game-Scripts-*BaseData : game-Scripts-*BaseData

---@type game-Scripts-RewardData*RewardSets
game.RewardSets = ...
---@alias game-Scripts-RewardData.RewardSets ...

return game
